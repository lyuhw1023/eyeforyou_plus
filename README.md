...
<img width="100" alt="splash" src="https://github.com/user-attachments/assets/5ce197ec-9509-449d-b869-62c856d568ca" />

<br><br>

## 🔗 업데이트 개요 (2025.03 기준)
> 이 프로젝트는 기존 EyeForYou 프로젝트를 개선하여 코드 구조를 최적화하고, MVVM 패턴 및 Provider를 적용하여 유지보수성을 향상시킨 버전입니다.
...
- 35개의 조건별 결과 페이지 → 1개의 동적 리스트 페이지로 통합
...
- OptionCard 및 BenefitCard 적용 → 선택 화면 및 혜택 정보 리스트 최적화
...
## 📌 프로젝트 기능 소개   
1. AI 카메라를 통한 상품 인식
> 사용자가 카메라 화면을 터치하면 AI가 상품을 인식하고 음성으로 안내  
> 상품 코너(스낵, 음료 등)까지 인식하여 사용자가 위치를 파악할 수 있도록 지원  
> AI 모델 최적화 → 4:3 비율 자동 크롭 적용하여 정확도 개선
...
2. 맞춤형 복지 혜택 정보 제공
> 중증/경증 + 생애주기/복지 서비스 선택 후 맞춤형 혜택 정보 제공  
> JSON 데이터를 활용하여 동적 필터링 및 리스트 구성  
> 웹 크롤링을 활용한 데이터 자동화
...
3. 접근성을 고려한 UI/UX
> VoiceOver 지원 → 시각장애인을 위해 음성 안내 기능 추가  
> Semantics 태그 적용 → 화면 요소를 음성으로 읽어줄 수 있도록 개선  
...
|항목|기존 프로젝트|개선된 프로젝트|
|:---:|:---:|:---:|
|메인화면 구조|메인, 로딩, 결과 화면이 하나의 파일에 혼합|각 화면을 역할별로 분리 및 ViewModel로 로직 관리|
|화면 구성|조건별 결과 화면이 35개 개별 파일로 구성|JSON 데이터 활용하여 1개의 동적 리스트 페이지로 통합|
|상태 관리|setState 과다 사용으로 성능 저하|Provider 적용으로 상태 관리 최적화
|데이터 입력|웹에서 수작업으로 데이터 복사/입력|웹 크롤링 도입하여 효율적으로 관리|
